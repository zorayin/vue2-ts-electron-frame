/**
 * 覆盖该文件后的功能(非必要)
 * 运行electron:serve，桌面端和web端不再同时运行,两端需要分别开发者新建终端窗口各自运行
 */
...
module.exports = (api, options) => {
  ...
  // cmd.runSync('npm run node-build')
  ...
  api.registerCommand(
    ...
    async (args, rawArgs) => {
      ...
      //注册密钥
      api.registerCommand('keygen', () => {
        const target = getPath('./src/key.txt')
        if (!fs1.existsSync(target)) {
          writeKey()
        } else {
      ...
      //生成入口node文件
      api.registerCommand('node-build', () => {
        let terser
        try {
        ...
        const cmdStr = 'npm run node-build';
        exec(cmdStr, (err, stdout, stderr) => {
          if (err) {
            log(err);
            warn(new Date(), ' 编译Node失败');
          } else {
            log(stdout);
            warn(new Date(), ' 编译Node成功');
            try {
              info("encrypt js file && update asar:");
              api.service.run('encrypt-asar');
            } catch (e) { error(e) }
          }
        });
      });
      ...
      //加密
      api.registerCommand('encrypt-asar', () => {
        // ../../src/electron
        // fs.readFile(path.join(__dirname, './dist/bundled/package.json'),'utf8',(err, data)=>{
        fs.readFile(getPath('./dist/bundled/package.json'), 'utf8', (err, data) => {
          if (err) throw err;
          let tempData = JSON.parse(data);
          if (tempData.main) {
            tempData.main = "main.node"
            let newContent = JSON.stringify(tempData, null, 4);
            // fs.writeFile(path.join(__dirname, './dist/bundled/package.json'), newContent, 'utf8', (err,data) => {
            fs.writeFile(getPath('./dist/bundled/package.json'), newContent, 'utf8', (err, data) => {
              if (err) throw err;
        ...
        const old_asarTarget = getPath('./dist/win-unpacked/resources/app.asar')
        fs.removeSync(old_asarTarget)

        const key = Buffer.from(fs1.readFileSync(getPath('./src/key.txt'), 'utf8').trim().split(',').map((v) => Number(v.trim())));
        const asarTarget = getPath('./dist/win-unpacked/resources/app.asar');

        asar.createPackageWithOptions(getPath('./dist/bundled'), asarTarget, {
          unpack: '*.node',
          transform(filename) {
        ...
        const target = getPath('./dist/win-unpacked/resources/node_modules.asar')
      ...
      //打包后重写部分，主要是将入口node化并加密
      async function overwrite(hasDone) {
        if (hasDone) {
          try {
            info('key generation:');
            await api.service.run('keygen').then(async () => {
              try {
                info("create node file:");
                await api.service.run('node-build').then(async () => {
                  //     try {
                  //       info("encrypt js file && update asar:");
                  //       await api.service.run('encrypt-asar');
                  //     } catch (e) { error(e) }
                });
              } catch (e) { error(e) }
            });
  ...
  api.registerCommand(
    ...
    async (args, rawArgs) => {
      ...
      // 重写这个链接解析，用于桌面端和web端不用运行再同一个终端上
      const tOptions = api.service.projectOptions;
      // const {host,port} = tOptions.devServer;
      const host = tOptions.devServer && tOptions.devServer.host ? tOptions.devServer.host : "localhost";
      const port = tOptions.devServer && tOptions.devServer.port ? tOptions.devServer.port : "8080";
      // const urls = prepareURLs(
      //   host,
      //   port,
      //   isAbsoluteUrl(tOptions.publicPath) ? '/' : tOptions.publicPath
      // )
...
function getPath(...args) {
  return require('path').join(__dirname, '../../src/electron', ...args)
}
